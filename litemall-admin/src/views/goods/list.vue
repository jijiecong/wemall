<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <!--      <el-button class="filter-item" type="primary" icon="el-icon-edit" @click="handleImport">导入</el-button>-->
      ...
      <el-button class="filter-item" type="danger" icon="el-icon-edit" :disabled="batchDeleteArr.length === 0" @click="handleOnSaleRows">批量上架</el-button>
      <el-button class="filter-item" type="danger" icon="el-icon-edit" :disabled="batchDeleteArr.length === 0" @click="handleOffSaleRows">批量下架</el-button>
    </div>
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" min-width="100" label="限购" prop="limitBuy" />
...
<script>
import { listGoods, deleteGoods, goodsOnSale, goodsOffSale } from '@/api/goods'
...
export default {
  ...
  methods: {
    ...
    handleImport() {
      this.$router.push({ path: '/goods/import' })
    },
    handleUpdate(row) {
    ...
    handleOffSale(row) {
      console.log('下架')
      goodsOffSale(row).then(response => {
        this.$notify.success({
          title: '成功',
          message: '下架成功'
        })
    ...
    handleOnSaleWithoutRefresh(row) {
      console.log('上架')
      goodsOnSale(row).then(response => {
        this.$notify.success({
          title: '成功',
    ...
    handleOffSaleWithoutRefresh(row) {
      console.log('下架')
      goodsOffSale(row).then(response => {
        this.$notify.success({
          title: '成功',
    ...
    handleOnSaleRows() {
      this.$confirm('确定批量上架?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        center: true
      }).then(() => {
        this.batchDeleteArr.forEach(row => this.handleOnSaleWithoutRefresh(row))
        this.getList()
      })
    },
    handleOffSaleRows() {
      this.$confirm('确定批量下架?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        center: true
      }).then(() => {
        this.batchDeleteArr.forEach(row => this.handleOffSaleWithoutRefresh(row))
        this.getList()
      })
    },
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['商品ID', '商品编号', '名称', '专柜价格', '当前价格', '是否新品', '是否热品', '是否在售', '排序', '限购', '首页主图', '宣传图片列表', '商品介绍', '详细介绍', '商品图片', '商品单位', '关键字', '类目ID', '品牌商ID']
        const filterVal = ['id', 'goodsSn', 'name', 'counterPrice', 'retailPrice', 'isNew', 'isHot', 'isOnSale', 'sort', 'limitBuy', 'listPicUrl', 'gallery', 'brief', 'detail', 'picUrl', 'goodsUnit', 'keywords', 'categoryId', 'brandId']
